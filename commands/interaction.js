module.exports = async (interaction, client) => {
    ...
    try {
        ...
        const cmds = client.cmd.default.filter((x) => x.name?.toLowerCase() === interaction.commandName?.toLowerCase()).V();
        ...
        for (const cmd of cmds) {
            ...
            if (cmd.name?.includes("$")) {
                cmd.name = (await client.functionManager.interpreter(client, data, [], { code: cmd.name, name: "NameParser" }, client.db, true, undefined, { interaction }))?.code;
            }
